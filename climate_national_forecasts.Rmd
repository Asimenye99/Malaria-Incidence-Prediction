...
We also need climate data with some lags. 

Maybe I should create a dataframe with temperature and dates.
Then I would get the same dates as the Malaria data.
Then I would need a rolling window to select the data from previous months.

```{r}

temperature <- read.csv("ERA5_climate_data/malawi_monthly_temperature.csv")

temperature_df <- data.frame(
  Date = as.Date(temperature$date),
  temp_no_lag = as.numeric(temperature$temperature),
  District = "National"
)

head(temperature_df)
```
Let's combine here both dataframes by date
I want to use the climate data on the ARIMAX model later

```{r}
national_incidence_temp <- national_incidence %>%
  left_join(
    temperature_df,
    by = c("Date", "District")
  )

head(national_incidence_temp)
```
...
```{r}
fit_one_district <- function(df, xreg_cols = NULL) {
  ...
  df2 <- right_join(df, tibble(Date = all_dates), by = "Date") %>%
    arrange(Date) %>%
    mutate(
      incidence_per_1k = replace_na(incidence_per_1k, 0)
    )
  
  # Handle covariates dynamically
  if (!is.null(xreg_cols)) {
    df2 <- df2 %>%
      mutate(across(all_of(xreg_cols),
                    ~ replace_na(.x, mean(.x, na.rm = TRUE))))
    
    xreg_mat <- as.matrix(df2[, xreg_cols])
  } else {
    xreg_mat <- NULL
  }
...
fit_nat <- fit_one_district(
  national_incidence_temp[1:60,],
  xreg_cols = "temp_no_lag"
)

future_temp <- national_incidence_temp$temp_no_lag[61:64]
future_xreg <- matrix(future_temp, nrow = 4)
colnames(future_xreg) <- fit_nat$xreg_cols
fc <- forecast(fit_nat$model, h = 4, xreg = future_xreg)
fc
...
autoplot(fc) +
  autolayer(fit_nat$ts, series = "Observed") +
  labs(
    y = "Malaria incidence per 1k",
    x = "Time",
    title = "Observed + Forecast (with prediction intervals)"
  ) +
  theme_minimal()
