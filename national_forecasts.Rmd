...
for (i in 1:27){ 
  ...
  exog_1lag_df <- rbind(
    exog_1lag_df,
    data.frame(lag = "lag1", h1 = exog1, h2 = exog1, h3 = exog1)
  )
  
  exog_2lag_df <- rbind(
    exog_2lag_df,
    data.frame(lag = "lag2", h1 = exog1, h2 = exog2, h3 = exog2)
  )
  
  exog_3lag_df <- rbind(
    exog_3lag_df,
    data.frame(window = "lag3", h1 = exog1, h2 = exog2, h3 = exog3)
  )
}
...
# Plot national forecast
#SARIMAX_forecasts <- forecast_baselines(sarimax_models, h = 3)
```
...
# Get quantiles
BASELINE_quantiles <- get_forecasted_quantiles(satimax_models_forecasts)
...
qdf <- as.data.frame(SARIMA_quantiles[[2]])
