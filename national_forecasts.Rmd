...
temp_exog_1lag <- lapply(
  1:nrow(temp_exog$lag1),
  function(i) {
    as.matrix(temp_exog$lag1[i, c("h1", "h2", "h3")])
  }
)

temp_exog_2lag <- lapply(
  1:nrow(temp_exog$lag2),
  function(i) {
    as.matrix(temp_exog$lag2[i, c("h1", "h2", "h3")])
  }
)

temp_exog_3lag <- lapply(
  1:nrow(temp_exog$lag3),
  function(i) {
    as.matrix(temp_exog$lag3[i, c("h1", "h2", "h3")])
  }
)
```
...
SARIMA_forecasts <- forecast_SARIMAs1(sarima_models, h = 3)

# Plot national forecast
BASELINE_forecasts <- forecast_baselines1(baseline_models, h = 3)


# Plot national forecast
sarimax_forecasts_3lag <- forecast_SARIMAs1(
  model_list = sarimax_models,
  h = 3, future_xreg_list = temp_exog_3lag
)

# Plot national forecast
sarimax_forecasts_2lag <- forecast_SARIMAs1(
  model_list = sarimax_models,
  h = 3, future_xreg_list = temp_exog_2lag
)

# Plot national forecast
sarimax_forecasts_1lag <- forecast_SARIMAs1(
  model_list = sarimax_models,
  h = 3, future_xreg_list = temp_exog_1lag
)
...
```{r}
# Get quantiles
SARIMA_quantiles <- get_forecasted_quantiles1(SARIMA_forecasts)

# Get quantiles
BASELINE_quantiles <- get_forecasted_quantiles1(BASELINE_forecasts)

# Get quantiles
SARIMAX_3_quantiles <- get_forecasted_quantiles1(sarimax_forecasts_3lag)

# Get quantiles
SARIMAX_2_quantiles <- get_forecasted_quantiles1(sarimax_forecasts_2lag)

# Get quantiles
SARIMAX_TEMP_1LAG <- get_forecasted_quantiles1(sarimax_forecasts_1lag)

```

Organize the quantiles in a way we may calculate WIS and MAE

```{r}

submission_1 <- format_forecast_for_WIS(SARIMAX_TEMP_1LAG)

formatted_baseline_forecasts<-format_forecast_for_WIS(BASELINE_quantiles)
...
my_forecast_scores<-covidHubUtils::score_forecasts(predictive_quantiles_tb, truth)

```

!
! Using temperature and rainfall, the baseline and the sarima
! we will have almost 10 models
!
! if we use a data transformation apporach we will have
! close to 20 models, which I think it will be enough
!
!
! I don't think we should use wind
!
! We may use some data transformations
!
!
! I need temperature and rainfall with no lag
!
! I need to forecast the correc 0 - 99 percentatilie similar to the other model
! 
! I need to check if the rolling windows are correct
!
! 
!
...
plot_forecast_window <- function(df, model_list, window_id, model_name, fc_date) {
  ...
  plot_df <- df %>%
    ...
    select(target_end_date, quantile, value) %>%
    tidyr::pivot_wider(
      names_from = quantile,
      values_from = value
    )

  train_df <- model_list[[window_id]]$data %>%
    select(Date, incidence_per_1k)

  ggplot() +
    # observed/training data
    geom_line(
      data = train_df,
      aes(x = Date, y = incidence_per_1k),
      linewidth = 0.8
    ) +
    geom_point(
      data = train_df,
    ...
    geom_ribbon(
      data = plot_df,
      aes(x = target_end_date, ymin = `0.1`, ymax = `0.9`),
      alpha = 0.3
    ) +
    geom_ribbon(
      data = plot_df,
      aes(x = target_end_date, ymin = `0.25`, ymax = `0.75`),
      alpha = 0.4
    ) +
    geom_line(
      data = plot_df,
    ...
    labs(
      x = "Date",
      y = "Incidence per 1k",
      title = paste(model_name, "forecast from", fc_date)
    )
}

plot_forecast_window(
  df = formatted_baseline_forecasts,
  model_list = baseline_models,
  window_id = 1,
  model_name = "BASELINE_quantiles",
  fc_date = "2022-12-01"
)
...
submission_1 %>%
  ...
  labs(
    x = "Target end date",
    y = "Median predicted incidence",
    color = "Forecast date",
    title = "Median forecasts by forecast window"
  )
