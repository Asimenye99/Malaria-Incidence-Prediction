...
rolling_sarima <- function(
  ...
  # name windows
  names(fitted_models_list) <- paste0("window_", seq_along(fitted_models_list))
  ...
  for (i in seq_along(model_list)) {
    ...
    fc_list[[i]] <- tryCatch({
      ...
      fc <- if (is.null(obj$xvar)) {
        ...
        window_name <- names(model_list)[i]
        
        if (is.null(future_xvar_list) || is.null(future_xvar_list[[window_name]])) {
          stop("Missing future xreg values")
        }
        # get one element from the future exogenous variables list
        future_xvar <- future_xvar_list[[window_name]]
        
        # remove the lag column
        future_xvar <- future_xvar[, c("h1", "h2", "h3")]
...
get_exog_variables <- function(data, # dataframe with incidence, date, temperature and rainfall
  ...
  future_exog_list <- vector("list", n_windows)
  for (i in 1:n_windows) {
    ...
    future_exog_list[[i]] <- row
  }
  
  names(future_exog_list) <- paste0("window_", seq_len(n_windows))
