...
climate_long <- full_dates %>%
  left_join(climate_long, by = "date")
...
year_start_dates <- unique(
  climate_long$date[format(climate_long$date, "%m") == "01"]
)

# -----------------------------
# 4. Plot (LINES ONLY + YEAR LABELS)
# -----------------------------
ggplot(climate_long, aes(x = date, y = value, group = variable)) +
  geom_line(color = "steelblue", linewidth = 0.9, na.rm = TRUE) +

  scale_x_date(
    breaks = climate_long$date,
    labels = climate_long$month_label,
    limits = c(as.Date("2018-01-01"), as.Date("2024-12-01"))
  ) +

  facet_wrap(~ variable, scales = "free_y", ncol = 1) +

  # ---- Year labels above each facet ----
  geom_text(
...
malaria_with_pop$district  # district names
malaria_with_pop$year
malaria_with_pop$total_incidence
